...
//				columns, KEY_UPLOADED_STATUS+" = ?", new String[]{status}, null, null, null);
    ...
    public int getToBeUploadeShelf(){
        ...
        Cursor cursor = database.rawQuery("select row_id from shelf_quantity where is_uploaded ='false'",null);
    ...
    public int getUploadedShelf(){
        openReadableDatabase();
        Cursor cursor = database.rawQuery("select row_id from shelf_quantity where is_uploaded ='true'",null);
        int count =cursor.getCount();
        cursor.close();
        closeDatabase();
        return count;
